...
<?php
	if(!isset($_POST["name"])) {
		...
		setcookie("utoptutorUser", $_POST["name"], time()+360000);
		setcookie("utoptutorEmail", $_POST["email"], time()+360000);
		setcookie("utoptutorTime", $curTimeTight, time()+360000);
	}
	...
	foreach ($_POST as $value) {
		if ($value!="Submit" and $value!=$name and 
			$value!=$email and $value!=$course) {
	...
	<div class="container">
		<div class="col-md-6 study-grid">
			...
			<div class="study1">
				...
				如果预订信息中出现"Failed to order"表明预定失败（可能是其他同学刚刚报名了这个时间，或者你重复提交了报名），请立刻联系客服（微信：UTopTutor）。辅导地点默认为BA二楼或三楼，具体位置将在辅导前一小时内通知。如需更改地点，有任何其他问题，也请联系客服。
			</div>
		</div>
		<div class="col-md-6 study-grid">
			...
			<div class="study2">
				...
				<?php
if($course == "CSC108" or $course == "CSC165" or $course == "CSC104" or 
   $course == "CSC148" or $course == "CSC120" or $course == "MAT137" or 
   $course == "MAT223" or $course == "MAT224") $unit = 30;
else if($course == "STA247" or $course == "MAT135" or $course == "MAT136") $unit = 20;
else if($course == "CSC108 (Group)") $unit = 40;
else $unit = 0;
if($unit == 0) echo '</ol>Please contact WeChat service for your fee.';
else echo $button_a.$unit.$button_b.$i.$button_c;
echo '<br><b>IMPORTANT:</b> Please make sure to return to this website after 
	  payment on PayPal.<br>Note: You will be auto directed back if you log 
	  into your PayPal account, or you will have to click on a link.</ol>';
